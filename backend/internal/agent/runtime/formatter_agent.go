...
func NewFormatterAgent(config *AgentConfig, modelClient ai.ModelClient, ragHelper *RAGHelper, promptEngine *prompt.Engine) *FormatterAgent {
...
func (a *FormatterAgent) Execute(ctx context.Context, input *AgentInput) (*AgentResult, error) {
	...
	resp, err := a.modelClient.ChatCompletion(ctx, &ai.ChatCompletionRequest{
		Messages:    messages,
		Temperature: a.config.Temperature, // 格式化通常使用较低的温度
