...
- 接口需返回 `status`、`reviewToken`、`nextActionDeadline` 等字段，前端基于此渲染 SLA。
- 暂存详情接口提供 `auditTrail[]`，与 `AuditEvent` 表相同字段，便于比对操作链路。
- 对于被驳回条目，系统要求填写 `resolutionChecklist`（例如“已修正命名”“上下文补充完成”），用于数据追踪。
...
### 8.2 容量规划与降级策略
- **容量假设**：
  - 单租户日均 5k 次命令执行、500 条暂存；
  - 文件树最高 100k 节点、峰值 20 并发编辑；
  - 暂存审核并发 50；命令队列默认每 Agent 10 并发。
- **扩容策略**：
  - 超过阈值 70% 时自动扩展 Agent Worker、副本数量或 Redis Stream 分片；
  - 文件索引用 Elastic/Meilisearch 水平扩容，采用热/冷节点区分。
- **降级级别**：
  1. 仅开放只读浏览（禁止写、命令调度），保留审核查看；
  2. 关闭高成本命令（如多文档上下文），仅允许轻量命令；
  3. 停用所有 Agent 调用，用户需手动编辑与提交。
- **灾备**：主存储每日全量备份 + 每小时增量，跨可用区存储；当归档写入失败超过 3 次时触发灾备切换流程，临时将 staging 改为长 TTL。
...
- 例外授权（Break Glass）：管理员可通过 `POST /api/admin/break-glass` 临时授予 30 分钟写权限，系统记录原因、审批人、自动到期时间并推送告警。
- 离线/隔离模式：当检测到潜在入侵或法规要求时，可切换为「本地隔离」模式，仅允许本地缓存读取与导出，所有 Agent 命令与外部回调被阻断，审计记录标记 `securityTags=isolated`。
